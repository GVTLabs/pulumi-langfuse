...
Pulumi SDKs for [Langfuse](https://langfuse.com), generated from
...
version. Tag `v0.8.0` here contains the SDKs generated from upstream `v0.8.0`.
Everything under `sdks/` is generated — never edit it by hand.

## Using the SDKs

The supported way to consume the Langfuse provider is to let Pulumi generate the
SDK into your own project, which is the same command this repository automates:

```bash
pulumi package add terraform-provider langfuse/langfuse
```

Pass a version as a trailing argument to pin to a specific upstream release:

```bash
pulumi package add terraform-provider langfuse/langfuse 0.8.0
```
...
```bash
go get github.com/gvtlabs/pulumi-langfuse/sdks/go@v0.8.0
```

```go
import "github.com/gvtlabs/pulumi-langfuse/sdks/go/langfuse"
```

The bridge stamps its own canonical path
(`github.com/pulumi/pulumi-terraform-provider/sdks/go/langfuse`) into every Go
SDK it generates, which matches no repository and so resolves nowhere. The sync
...
The provider takes the same settings as the Terraform provider — the base URL of
the Langfuse instance and an admin API key. The admin key is only needed when
managing organizations and organization API keys, which requires an Enterprise
license on the Langfuse side:

```bash
pulumi config set langfuse:host https://cloud.langfuse.com
pulumi config set --secret langfuse:adminApiKey <LANGFUSE_ADMIN_KEY>
```

The admin key can also come from the `LANGFUSE_ADMIN_KEY` environment variable.
Project-scoped resources authenticate with organization keys passed as resource
inputs rather than through provider configuration.

See the [upstream provider documentation](https://registry.terraform.io/providers/langfuse/langfuse/latest/docs)
for the full resource reference. Resource names are the Terraform names in each
language's idiomatic casing, so `langfuse_organization_api_key` becomes
`OrganizationApiKey`.
...
```bash
gh api repos/gvtlabs/pulumi-langfuse/dispatches \
  -f event_type=upstream-release \
  -F 'client_payload[version]=0.8.0'
```
...
./scripts/sync.sh --version 0.7.0     # backfill a specific version
