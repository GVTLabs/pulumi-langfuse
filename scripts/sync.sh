#!/usr/bin/env bash
#
# Mirror a terraform-provider-langfuse release as Pulumi SDKs.
#
...
UPSTREAM_REPO="langfuse/terraform-provider-langfuse"
TF_PROVIDER="langfuse/langfuse"
EX_TEMPFAIL=75
...
GO_MODULE_CANONICAL="github.com/pulumi/pulumi-terraform-provider/sdks/go/langfuse"
GO_MODULE_PATH="github.com/gvtlabs/pulumi-langfuse/sdks/go"
...
if tag_exists "$tag" && [[ "$force" != true ]]; then
  ...
  git tag -a "$go_tag" "$tag^{}" -m "terraform-provider-langfuse $tag"
...
plugin_json="$staging/python/pulumi_langfuse/pulumi-plugin.json"
...
if git diff --cached --quiet; then
  ...
  git commit -m "Generate SDKs for terraform-provider-langfuse $tag
...
for t in "$tag" "$go_tag"; do
  if tag_exists "$t"; then
    ...
    git tag -a "$t" -m "terraform-provider-langfuse $tag"
    log "Tagged $t"
  fi
done
